 ...
 * LCD.c
 *
 * Created: 08-10-2025 20:16:41
 *  Author: MinnuDiary
 */ 
#include "lcd.h"
#include <util/delay.h> // I added 
...
{
	while (*s) // checks whether \0 reached or not  (string ended or not )
	{
		lcd_data((uint8_t)*s++); // Type casting  (ASCII is sending till ending occurs ) ; lcd data is a function defined above
	}
...
{
	char buffer[16];  // enough for "12345.6789"
	sprintf(buffer, "%.2f", num);   // format with 2 decimal places
	lcd_print(buffer);
}
